...
import com.adiacent.menarini.guidotti.lab.core.utils.Constants;
...
@Component(service = Servlet.class, name = "Laboratorio guidotti - EFPIA Servlet",  immediate = true)
@SlingServletResourceTypes(
        resourceTypes = "laboratorio-guidotti/components/efpia" ,
        methods= {HttpConstants.METHOD_GET},
        extensions= Constants.JSON,
        selectors={EfpiaServlet.DEFAULT_SELECTOR})
@ServiceDescription("Laboratorio guidotti - EFPIA Servlet")
        ...
        try{
            ...
            if(currentResource.getValueMap().containsKey("folderPath")){
                ...
                if(damFolder.getResourceType().equals("sling:Folder") || damFolder.getResourceType().equals("nt:folder")){
                    ...
                    Year yearBeforeLast = currentYear.minusYears(2);
                    Year threeYearsAgo = currentYear.minusYears(3);

                    String lastyeardampath = damFolderPath+"/"+lastYear;
                    String yearbeforelastdampath = damFolderPath+"/"+yearBeforeLast;
                    ...
                    if(resourceResolver.getResource(yearbeforelastdampath) != null){
                        JsonArray jsonArray = getResult(yearbeforelastdampath, resourceResolver);
                        if(jsonArray.size() > 0){
                            finalResult.add(String.valueOf(yearBeforeLast),jsonArray);
                        }
