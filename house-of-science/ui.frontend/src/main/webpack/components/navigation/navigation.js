...
jQuery(function ($) {
  ...
  var HEADER_CONTAINER_SELECTOR = ".cmp-experiencefragment--header",
    ...
    HEADER_NAVIGATION_SELECTOR = ".navigation .cmp-navigation",
    ROOT_LEVEL_NAVIGATION_SELECTOR =
      ".cmp-navigation__item--level-0.cmp-navigation__item--active > .cmp-navigation__item-link",
  ...
  function createMobileNavigation() {
    ...
    if (
      $nav !== undefined &&
      $nav.length === 1 &&
      !$($nav)[0].hasAttribute(DATA_PROCESSED)
    ) {
      ...
      $(
        '<div id="toggleNav">' +
          '<a href="" aria-label="Open hidden mobile navigation" class="cmp-button--mobile">' +
          "<span></span>" +
          "<span></span>" +
          "<span></span>" +
          "</a>" +
    ...
    $(".cmp-button--mobile").on("click", function (e) {
      e.preventDefault();
      if (!$(this).hasClass("toggle")) {
        $(this).addClass("toggle");
      }
      $(".toggle").toggleClass("cmp-button--mobile__open");
      $($body).toggleClass("no-overflow");
      $("#mobileNav").toggleClass("cmp-navigation--mobile__open");
    });
  }
  ...
  setHomeLink(
    HEADER_NAVIGATION_SELECTOR + " " + ROOT_LEVEL_NAVIGATION_SELECTOR,
    HEADER_CONTAINER_SELECTOR + " " + LOGO_SELECTOR
  );
  setHomeLink(
    FOOTER_CONTAINER_SELECTOR + " " + ROOT_LEVEL_NAVIGATION_SELECTOR,
    FOOTER_CONTAINER_SELECTOR + " " + LOGO_SELECTOR
  );
});
