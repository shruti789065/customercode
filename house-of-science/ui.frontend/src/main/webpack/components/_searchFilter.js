...
function filterDataFromCookies() {
  ...
  const displayData = (data) => {
    ...
    data.forEach((item) => {
      ...
      html += `
      <div class="cardSFR">
        <div class="card-header" id="heading-${item.typology}-${item.date.replace(/\//g, '-')}">
            <button class="btn btn-link collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#collapse-${item.typology}-${item.date.replace(/\//g, '-')}" aria-expanded="true" aria-controls="collapse-${item.typology}-${item.date.replace(/\//g, '-')}">
              <span class="typology">${item.typology}</span>
              <span class="description">${item.description}</span>
            </button>
            <span class="date">${item.date}</span>
        </div>
        <div id="collapse-${item.typology}-${item.date.replace(/\//g, '-')}" class="collapse" aria-labelledby="heading-${item.typology}-${item.date.replace(/\//g, '-')}" data-bs-parent="#results">
          <div class="card-body">
            <div class="meta-data">
