...
$(function () {
  const SearchMedicine = {
    ...
    initializeVariables() {
      ...
      this.farmacovigilanzaForm = document.querySelector(
        "#farmacovigilanzaForm"
      );
    },

    callPIMAPI(query, currentPageSearch) {
      const endpoint = `${currentPageSearch}.searchMedicineResult.json?medicineName=${query}`;
          ...
          radioBtn = data.name
            .map(
              (
                result,
                index
              ) => ` <input type="radio" name="product" value="${result}" ${
                index === 0 ? "checked" : ""
              }>
    ...
    performSearch() {
      ...
      if (this.query != "") {
        this.callPIMAPI(this.query, this.currentPageSearch);
      }
    },

    radioBtnAction() {
      let radioBtns = document.querySelectorAll(
        ".farmacovigilanzaResult input[type='radio']"
      );
      if (radioBtns) {
