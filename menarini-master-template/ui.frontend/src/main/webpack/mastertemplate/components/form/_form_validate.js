...
$(() => {
  ...
  if ($form !== null) {
    ...
    $form.querySelectorAll("[required]").forEach((item) => {
      ...
      if (item.id !== "") {
        document
          .querySelector("label[for=" + item.id + "]")
    ...
    $fileInput.on("change", function () {
      ...
      if (file.size > 3 * 1024 * 1024) {
        ...
        $fileInputParent.append(
          '<p class="label-error label_file_too_big">File too big: Limit is 3MB</p>'
        );
      } else {
        $fileInputParent.find(".label_file_too_big").remove();
      }
      ...
      if (!fileExtensionsAllowed.includes(fileNameExt)) {
        ...
        $fileInputParent.append(
          '<p class="label-error label_file_extension_not_allowed">File extension not allowed</p>'
        );
      } else {
        $fileInputParent.find(".label_file_extension_not_allowed").remove();
      }
    ...
    $filesContainer.on("click", ".cmp-close__icon", function () {
      ...
      if (
        document.querySelector(
          ".cmp-form-text .label_file_extension_not_allowed"
        ) !== null
      ) {
        document
          .querySelector(".cmp-form-text .label_file_extension_not_allowed")
          .remove();
      }
    });
...
function allValidation() {
  ...
  //return inputsAreValid && radiosAreValid;
}
          ...
          //se non è select
            ...
            if ($(this).attr("type") == "file") {
              ...
              //se ha contenuto viene fatta la validazione del file
                ...
                var fileNameExt = file.name.substring(
                  file.name.indexOf(".") + 1
                );
                ...
                if (file && file.size < 3 * 1048576) {
                  ...
                  if (
                    $(this).parent().find(".label_file_too_big").length == 0
                  ) {
                    $(this)
                      .parent()
                      .append(
                ...
                if ($.inArray(fileNameExt, fileExtensionsAllowed) !== -1) {
                  if (
                    $(this).parent().find(".label_file_extension_not_allowed")
                      .length
                  ) {
                    $(this)
                      .parent()
                      .find(".label_file_extension_not_allowed")
                      .remove();
                  }
                  //se estensione nok
                } else {
          ...
          if ($(this).attr("type") == "email") {
            ...
            if (email.length > 0) {
              ...
              //se esiste anche la label "required" la toglie
                ...
                $(this).css("border", "3px solid #a94442");
                if (
                  ($(this).parent().find(".label_email_not_valid").length ==
                    0) &
                  ($(this).parent().find(".label_required").length == 0)
                ) {
                  $(this)
                    .parent()
                    .append(
                      "<p class='label-error label_email_not_valid'>Please enter a valid email address</p>"
                    );
                  if ($(this).parent().find(".label_required").length) {
                    $(this).parent().find(".label_required").remove();
                  }
          ...
          if (isRequired == 1) {
            ...
            if ($(this).parent().siblings(".label_required").length == 0) {
              $(this)
                .parent()
                .after(
                  `<p class='label-error label_required'>${$(this)
                    .closest("fieldset")
                    .attr("data-cmp-required-message")}</p>`
                );
            }
          }
...
function _getConstrainMessage(el) {
  return `<span class='label-error label_required'>${el.parentElement.getAttribute(
    "data-cmp-required-message"
  )}</span>`;
}
