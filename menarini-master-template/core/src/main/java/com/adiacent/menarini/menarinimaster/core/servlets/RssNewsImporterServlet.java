...
@Component(service = { Servlet.class }, name = "Menarini Master template - Import RSS News Servlet", immediate = true)
@SlingServletResourceTypes(
        resourceTypes = NameConstants.NT_PAGE,
        methods= {HttpConstants.METHOD_GET},
        extensions= "json",
        selectors={RssNewsImporterServlet.DEFAULT_SELECTOR})
@ServiceDescription("Menarini Master template - Import RSS News Servlet")
public class RssNewsImporterServlet extends BaseJsonServlet{
    private final transient Logger logger = LoggerFactory.getLogger(this.getClass());
    public static final String DEFAULT_SELECTOR = "rssNewsImporter";
    protected transient RssNewsImporterServlet.Response ilr = null;

    @Reference
    protected transient RssNewsImporter importerInstance;
    ...
    protected void doGet(SlingHttpServletRequest request, SlingHttpServletResponse response) {
        ...
        if (running) {
            ...
            if (endImport != null && endImport.before(Calendar.getInstance()) && Calendar.getInstance().before(waitingInterval)) {
                ...
                ilr = new RssNewsImporterServlet.Response();
                running = true;
                try {

                    RssNewsImporter clonedImporterInstance = (RssNewsImporter) importerInstance.clone();
            ...
            sendResult(ilr, response);
            logger.info("******************FINE Rss News Import servlet ************************");
        }
    }
