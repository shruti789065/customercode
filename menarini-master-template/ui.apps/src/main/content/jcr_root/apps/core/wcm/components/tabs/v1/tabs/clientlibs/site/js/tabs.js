...
(function () {
	...
	function Tabs(config) {
		...
		function init(config) {
			...
			that._active = getActiveIndex(that._elements["tab"], that._config.element.id);
				...
				if (Array.isArray(tabpanels)) {
					for (var i = 0; i < tabpanels.length; i++) {
						if (i === parseInt(that._active) && !that._clickItself) {
							...
							document.querySelector(".cmp-share__container").style.display = "none";
						}
					}
				} else {
					// only one tab
		...
		function navigate(index) {
			...
			if (that._elements.tab[index].getAttribute('tabindex') == '0') {
				that._active == index ? that._clickItself = true : that._clickItself = false;
	...
	function onDocumentReady() {
		...
		var tabpanels = Array.from(document.querySelectorAll('.cmp-tabs__tabpanel.tabs-menu__container'));
		var tabs = Array.from(document.querySelectorAll('.cmp-tabs__tablist .cmp-tabs__tab'));
		...
		document.querySelector('.cmp-navbar-overlayer').addEventListener('click', () => {
			_closeShare();
		 ...
		 */
		 ...
		 if(document.querySelectorAll('.cmp-container__general-megamenu').length > 0){
			document.querySelector('.cmp-share .cmp-button').addEventListener('click', () => {
				_closeMenu(tabpanels, tabs);
			});
		 }
